...
try {
    ...
    for ($sample = 0; $sample -lt 2; $sample++) {
        ...
        $samples += [ordered]@{ utc=[DateTimeOffset]::UtcNow.ToString('o'); stage=$Stage; cpuPercent=$cpu; gpu=$gpu; freeHostBytes=$freeHost; diskFreeBytes=$disk; foreignHeavy=$foreign }
