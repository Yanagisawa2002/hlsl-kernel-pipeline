...
& $ValidationLockScript -Action {
    $sourceCommit = (& git -C $evidenceRepo rev-parse HEAD)
    $sourceStatus = (& git -C $evidenceRepo status --porcelain)
    $sourceFiles = @(& git -C $evidenceRepo ls-files -- src kernels tools manifests schemas Directory.Build.props | Where-Object { $_ -notmatch '(^|/)(bin|obj)/' })
    $sourceHashes = @($sourceFiles | ForEach-Object {
        [ordered]@{ path = $_; sha256 = (Get-FileHash -LiteralPath (Join-Path $evidenceRepo $_) -Algorithm SHA256).Hash }
    })
    ...
    $metadata = [ordered]@{
        ...
        buildSourceStatus = $sourceStatus
        buildSourceFiles = $sourceHashes
    ...
    $changedDuringRun = @($sourceHashes | Where-Object {
        (Get-FileHash -LiteralPath (Join-Path $evidenceRepo $_.path) -Algorithm SHA256).Hash -ne $_.sha256
    })
    $metadata['sourceUnchangedDuringRun'] = $changedDuringRun.Count -eq 0
    ...
    if ($changedDuringRun.Count -gt 0) { throw 'Source changed during evidence execution; receipt is not eligible.' }
}
