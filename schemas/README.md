...
- `profile.schema.v3.json`: independently confirmed profile and deployment contents.
- `profile.schema.v2.json`: historical profile, accepted only with explicit opt-in.
- `checkpoint.schema.v2.json`: paired session checkpoint; interrupted timing is archived.
- `checkpoint.schema.v1.json`: historical candidate-granular measurement state.
